...
if(isset($_REQUEST['showAttendanceReport']) && isset($_REQUEST['report_type']) && $_REQUEST['report_type']=='CONSOLIDATE'){
    ...
    $atn_consolidate_report = getInternsAttendanceConsolidateReportOfIntern($from_date, $to_date,$batch_id_from,$userId);
...
//    <link href='../assets/bsb/plugins/bootstrap/css/bootstrap.css' rel='stylesheet'>";
    echo "<style>
		th {
		  position: relative;
		}
...
//        while ($rowm = $resm->fetch_assoc()) {
        ...
        $getper = round(($acr["t_present"]/$acr["t_rows"])*100,0);
        if($getper<75){
...
echo "<br>";
    echo " <!-- Pie Chart -->
                            <canvas id='pie_chart' height='75'></canvas>
                <!-- #END# Pie Chart -->";

    echo "<!-- Jquery Core Js -->
    <script src='../assets/bsb/plugins/jquery/jquery.min.js'></script>
    <!-- Chart Plugins Js -->
    <script src='../assets/bsb/plugins/chartjs/Chart.bundle.js'></script>
    ...
    if (type === 'pie') {
        config = {
            ...
            data: {
                datasets: [{
                    data: [ $acr[t_present] , $acr[t_absent] ],
                    backgroundColor: [
                        'rgb(139, 195, 74)',
                        'rgb(233, 30, 99)'
                    ],
...
<table border='1' cellpadding='5'>";
    ...
    $bi = getPartInternsForAttendance($intern_id);
